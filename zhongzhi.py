...
# import pytesseract
# TODO:
# 1.base识别后，经常性无法点击到第一个地块（可能是定位有偏差）
...
def findElement(img_path, conf=0.6, notice=False):
    ...
    d.screenshot("shot.png")
    screen = cv2.imread("shot.png", 0)  # 灰度读取
    if 'first_page_sign' in img_path:
        d.screenshot("first_page.png")
        screen = cv2.imread("first_page.png", 0)  # 灰度读取
    for img_i in os.listdir(img_path):
        try:
            template = cv2.imread(img_path + img_i, 0)  # 目标按钮的图片
            res = cv2.matchTemplate(screen, template, cv2.TM_CCOEFF_NORMED)
    ...
    for img_i in os.listdir(img_path):
        try:
            ...
            if max_val >= conf:
                ...
                # 点击
                print("找到{0}，坐标为: {1},{2}".format(img_i, center_x, center_y))
...
def sell(earn_money):
    ...
    time.sleep(0.5)
    mouseClick('pictures/common/store_small/', conf=0.5) #点击商店
    for sell_item in sell_items: #逐样售卖待售物品
        # time.sleep(5)
        earn_money += sell_goods('pictures/' + sell_item + '/goods_forsale/', earn_money) #这一步填满货架
    # 此时不存在可操作的售空货架或者空货架，关闭自己的商店
    # mouseClick('pictures/common/close_store/')
    print('累计赚钱{0}'.format(earn_money))
    time.sleep(1)
...
def sell_goods(goods_forsale, earn_money):
    #按照最贵的价格销售一波指定商品
    time.sleep(1)
    #只销售一波
    if findElement('pictures/common/shelf_sold/'): 
        ...
        print('已售货架有{0}个'.format(len(shelves_sold)))
        earn_money += len(shelves_sold) * 36
        for shelf_sold in shelves_sold:
            # print(f"售空货架中心坐标为: {center}")
            d.click(int(shelf_sold[0]), int(shelf_sold[1]))
        time.sleep(2)
        shelves_sold_after = locateAllOnScreen('pictures/common/shelf_sold/', conf=0.8)
        shelves_sold_after = clear_matches(shelves_sold)
        if len(shelves_sold_after) >= len(shelves_sold): #点完后已售货架没有变少，大概率是卡住了
            return earn_money
    ...
    if findElement('pictures/common/shelf_empty/'):
        ...
        for k in range(len(shelves_empty)):
            ...
            time.sleep(0.5)
    else: #没有空货架，等待广告cd（避免供过于求的情况）
        d.click(300,330) #点击第一个货架
        time.sleep(1)
        if findElement('pictures/common/ad_avaliable/'): #广告cd完成
            d.click(990,415)
            time.sleep(1)
            mouseClick('pictures/common/ad_build/')
            time.sleep(60*2) #等两分钟再看
            earn_money += sell_goods(goods_forsale, earn_money)
        else: #广告cd未完成
            d.click(1140,70) #关闭创建广告窗口 
            time.sleep(60*2) #等两分钟再看
            earn_money += sell_goods(goods_forsale, earn_money)
...
def restart():
    ...
    # 格式化输出
    print("重启时间:", now.strftime("%Y-%m-%d %H:%M:%S"))  # 格式化为常见格式
    ...
    # # 关闭应用(g关闭应用再开启的办法，无法将雷电模拟器置于前台，导致后续pyautogui f5键按下失灵)
    ...
    slip_times = 0
    # 下滑至指定位置（商场-邮箱那里）
    while not findElement('pictures/common/mail_small/'):
        width, height = d.window_size()
        print('当前屏幕尺寸为：宽{0}x高{1}'.format(width, height))
        d.drag(width*1/4,height*3/4, width/2,height/2, duration=0.3)
        time.sleep(1)
        slip_times += 1
        if slip_times >= 10: #避免死循环
            break
    
    reset()

def close_honor():
    if findElement('pictures/common/close_notice/'):
        d.click(1400,80)
        time.sleep(1)
        print('关闭成就榜成功！')
...
def mix_path(raw_path_1, raw_path_2):
    #混合路径
    new_path = []
    for i in range(len(raw_path_1)):
        new_path.append(raw_path_1[i])
        new_path.append(raw_path_2[i])

    return new_path
def plant():
    #种植作物
    location_plant = locateOnScreen('pictures/zhongzhi_small/'+plant_items[0]+'_plant/',conf=0.7)
    print('plant所在位置为：[{0}, {1}]'.format(location_plant[0], location_plant[1]))

    # 移动到种植作物上并按下鼠标左键
    d.touch.down(location_plant[0], location_plant[1])  #点击种植作物

    # 按顺序经过每个点
    location_base = locateOnScreen(base_path, conf=0.5) # 重新定位地标，因为点击种植作物后，整个屏幕会调整位置
    point_1 = [location_base[0]+cell[0]*2, location_base[1]-cell[1]*4]
    point_2 = [location_base[0], location_base[1]-cell[1]*2]
    points_up = [
        [point_1[0]+i*cell[0]*0.5, point_1[1]+i*cell[1]*0.5] for i in range(9)
    ]
    point_low = [
        [point_2[0]+i*cell[0]*0.5, point_2[1]+i*cell[1]*0.5] for i in range(9)
    ]
    points = mix_path(points_up, point_low)
...
def gain():
    #收获作物
    #按住镰刀按照固定轨迹拖，松开镰刀
    location_sickle = locateOnScreen('pictures/zhongzhi_small/sickle/',conf=0.6)
    ...
    # 按顺序经过每个点
    location_base = locateOnScreen(base_path,conf=0.5)
    point_1 = [location_base[0]+cell[0]*2, location_base[1]-cell[1]*4]
    point_2 = [location_base[0], location_base[1]-cell[1]*2]
    points_up = [
        [point_1[0]+i*cell[0]*0.5, point_1[1]+i*cell[1]*0.5] for i in range(9)
...
def reset():
    # 将邮箱拉回原来的位置
    location = locateOnScreen(mail_path, conf=0.6)
    d.drag(location[0], location[1], mail_suitable[0],mail_suitable[1], duration=0.1)
    time.sleep(2) #防止屏幕拖动的缓冲影响后续因素定位，先等图片缓冲结束
    if checkFirstPage(): #防止误开邮箱
        d.click(1490, 40)

    # location_base = locateOnScreen(base_path, conf=0.5)
    # d.drag(location_base[0], location_base[1], base_suitable[0],base_suitable[1], duration=0.05)
    # time.sleep(2) #防止屏幕拖动的缓冲影响后续因素定位，先等图片缓冲结束

def take_action():
    # 将种植和收获动作统一到一个函数中，增加动作执行的弹性
    break_flag = False
    mouseClick(base_path, conf=0.5) #将光标转移到地标上
    time.sleep(1)
    location_base = locateOnScreen(base_path, conf=0.5)
    print('地块的位置是{0}， {1}'.format(location_base[0]+cell[0]*2+bias[0], location_base[1]-cell[1]*4+bias[1]))
    d.click(location_base[0]+cell[0]*2+bias[0], location_base[1]-cell[1]*4+bias[1]) #点击第一排最右边的土地，这时屏幕会调整位置
    time.sleep(2)
    
    if findElement('pictures/zhongzhi_small/'+plant_items[0]+'_plant/'):
        plant()
        reset()
        time.sleep(mature_time+5)
    elif findElement('pictures/zhongzhi_small/sickle/'):
        gain()
        ...
        d.screenshot("error_shot.png") #保存错误截图
        restart() #进入下一个循环
...
if __name__ == '__main__':
    ...
    # print(d.info)
    # scaling = 1600/2560
    mail_suitable = [605, 637]
    base_suitable = [815, 665]
    bias = [0, 25] #base元素中心点和其下地块有一定偏移,需要进行修正
    cell = [96, 48] #地块的宽高尺寸
    base_path = 'pictures/zhongzhi_small/base/'
    mail_path = 'pictures/common/mail_small/'
    ...
    # while False:
        ...
        # while True:
            time.sleep(5) #设定久一点，以免上一个循环收割的小麦遮挡地标
            
            break_flag = take_action()
            if break_flag:
                break
                    
        earn_money += sell(earn_money)
